...
// ========== DRAG AND DROP FUNCTIONALITY (CORRECTED) ==========
...
/**
 * Handle drag start event
 * ✅ FIXED: Changed from "this" to "e.currentTarget"
 */
function handleDragStart(e) {
    draggedElement = e.currentTarget;  // ✅ FIXED
    draggedType = draggedElement.classList.contains('rule-group') ? 'rule-group' : 'rule';
    ...
    setTimeout(() => {
        if (draggedElement) {  // ✅ FIXED: Added safety check
            draggedElement.classList.add('dragging');
        }
    }, 0);
...
function handleDragOver(e) {
    ...
    const container = e.currentTarget;  // ✅ FIXED
    const afterElement = getDragAfterElement(container, e.clientY);
    
    if (draggedType === 'rule-group') {
        const groupsContainer = document.getElementById('groupsContainer');
        if (afterElement == null) {
            groupsContainer.appendChild(draggedElement);
        } else {
            groupsContainer.insertBefore(draggedElement, afterElement);
        }
    } else if (draggedType === 'rule') {
        // For billing rules, only work within the same .rules container
        const rulesContainer = container.classList.contains('rules') ? container : container.closest('.rules');
        if (rulesContainer && sourceContainer === rulesContainer) {
            if (afterElement == null) {
...
/**
 * Handle drag end event
 * ✅ FIXED: Changed from "this" to use draggedElement variable
 */
function handleDragEnd(e) {
    if (draggedElement) {  // ✅ FIXED
        draggedElement.classList.remove('dragging');
    }
